...
import { Globe2, Mail, ArrowRight, Loader2 } from 'lucide-react'
        ...
        <div className="bg-gray-900/80 border border-gray-800 rounded-2xl p-6 shadow-2xl">
          {sent ? (
            <div className="text-center space-y-4">
              <p className="text-green-400 text-sm">✓ Reset code sent to <strong>{email}</strong></p>
              <p className="text-gray-400 text-sm">Check your inbox, then enter the code on the next page.</p>
              <button onClick={() => router.push('/update-password')} className="btn-primary w-full">
                Enter reset code →
              </button>
            </div>
          ) : (
            <>
              ...
              <div className="text-center mt-4">
                <a href="/login" className="text-sm text-brand-400 hover:text-brand-300">Back to login</a>
              </div>
            </>
          )}
        </div>
